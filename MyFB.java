...
public class MyFB {
	...
	static class Person {
		...
		public boolean equals(Object obj) {
			...
			Person p = (Person) obj;
			return ((this.age == p.age || ((Integer) this.age).equals(p.age))
					&& (this.name == p.name || this.name.equals(p.name)));
		}
	...
	// addFriendship does nothing if p1 and p2 are already friends or if one does
	// not exist
	public void addFriendship(Person p1, Person p2) {
		// TODO

		if (!map.containsKey(p1) || !map.containsKey(p2) || p1.equals(p2))
			return;

		if (!map.get(p1).contains(p2))
			map.get(p1).add(p2);
		if (!map.get(p2).contains(p1))
			map.get(p2).add(p1);

		if (DEBUG) {
			StdOut.format("F %s %s: %s\n", p1, p2, map);
		}

	}

	// removeFriendship does nothing if p1 and p2 are not friends or if one does not
	// exist
	public void removeFriendship(Person p1, Person p2) {
		...
		if (DEBUG) {
			StdOut.format("U %s %s: %s\n", p1, p2, map);
		}
	}

	// queryFriendship returns false if p1 and p2 are not friends or if one does not
	// exist
	public boolean queryFriendship(Person p1, Person p2) {
		...
		return map.get(p1).contains(p2) && map.get(p2).contains(p1);
	}
	...
	public static void main(String[] args) {
		...
		/*
		 * The first line below causes "in" to read from the console. You can change
		 * this to read from a file, by using something like the line below, where
		 * "input.txt" is a file you create in your eclipse workspace. The file should
		 * be in the same folder that contains "src" "bin" and "lib":
		 */

		// In[] inputFiles = new In[] { new In () }; // from console
		// In[] inputFiles = new In[] { new In("input.txt") }; // from file
		In[] inputFiles = new In[] { new In("input.txt"), new In() }; // from file,
		// then console
		...
		for (In in : inputFiles) {
			while (in.hasNextLine()) {
				...
				// StdOut.println (action); // while debugging, print debugging info here! You
				// can print maps, sets, lists.
