...
public class CSC402Homework4 {
	...
	public static int sumOfOddsRecursive (int[] a) {
		...
		if (a.length == 1 && a[0] % 2 != 0)           
			return a[0]; 
		return sumOfOddsHelper(a, a.length, 0); // TODO 1  replace this by a call to your helper function, then write the helper function below
		
	}
    // this would be a good place to put the helper function for #1
	public static int sumOfOddsHelper(int[] a, int b, int sum)
	{
		if ( b - 1 >= 0 && a[b-1] % 2 != 0)
			sum += a[b-1];
		if(b - 1 >= 0)
 ...
 */
	public static void reverseArray (int[] a) {
		...
		reverseArrayHelper(a,a.length - 1, 0,0);  // TODO 2   replace this by a call to your recursive helper function, then write the helper function below
		return;
	}
private static int[] reverseArrayHelper(int[] a,int hiIndex, int loIndex, int save) {
	...
	if(loIndex + 1 >= a.length)
		 reverseArrayHelper(a, hiIndex--, loIndex++,0);  
return a;
}
